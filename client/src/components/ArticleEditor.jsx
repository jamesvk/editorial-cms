...
export default function ArticleEditor() {
  ...
  const inputClass = 'mt-1 block w-full rounded border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-400';
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-widest mb-6">Editor</h2>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <label className="block">
            <span className="text-sm font-medium text-gray-700">Publish At</span>
        ...
        <fieldset className="border border-gray-200 rounded p-3">
          <legend className="text-sm font-medium text-gray-700 px-1">Tags</legend>
          <div className="flex flex-wrap gap-2 mb-3">
            {draft.tags?.map((tag) => (
              <span key={tag} className="flex items-center gap-1 bg-gray-100 text-gray-700 text-xs rounded px-2 py-1">
                {tag}
                <button
                  type="button"
                  onClick={() =>
